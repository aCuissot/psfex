...
int	cplot_init(char *name, cplotenum cplottype) {
	...
	*pstr = '\0';
	...
	plotaaflag = 0;
	if (cplot_device[j].device == CPLOT_PNG || cplot_device[j].device == CPLOT_JPEG) {
	...
	plfontld(1);
	plscolbg(255,255,255);	/* Force the background color to white */
	plscol0(15, 0,0,0);		/* Force the foreground color to black */
	plscol0(3, 0,200,0);		/* Force the green color to darken */
	plscol0(7, 128,128,128);	/* Force the midground color to grey */
	plscol0(8, 64,0,0);		/* Force the brown color to darken */
	plinit();
